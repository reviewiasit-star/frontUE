  ...
  if (userRole === "Cajero") {
    return [
      ...ejemplosComunicados.slice(0, 3),
      {
        ...
        query:
          "Necesito consultar el estado de pagos de un estudiante por nombre o CI.",
      },
    ];
  }
    ...
    try {
      ...
      if (useLangChain) {
        ...
        if (!resp.ok) {
          ...
          const msgError =
            errData.message || "Error al comunicarse con el agente";
          const detalle = errData.error ? `\nDetalle: ${errData.error}` : "";
          throw new Error(msgError + detalle);
        }
      ...
      setError(
        err.message ||
          "No se pudo conectar con el agente. Verifica tu conexión.",
      );
    } finally {
      setCargando(false);
    }
  ...
  const descargarDocumentoAsistencia = async (doc) => {
    try {
      ...
      const resp = await fetch(
        `${baseUrl}/api/ai-admin-langchain/generar-asistencia`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!resp.ok) {
        const errBody = contentType.includes("application/json")
          ...
          : {};
        throw new Error(
          errBody.message || `Error ${resp.status} al generar el documento`,
        );
      }
      const blob = await resp.blob();
      if (blob.type && blob.type.includes("application/json")) {
        const errData = await blob.text().then((t) => {
          try {
            return JSON.parse(t);
          } catch {
            return {};
        ...
        throw new Error(
          errData.message || "El servidor no devolvió un archivo",
        );
      }
      const ext =
        doc.formato === "word" || doc.formato === "docx" ? "docx" : "pdf";
    ...
    <>
              ...
              <div ref={chatContainerRef} className="ai-chat-container">
                {conversacion.length === 0 ? (
                  <div className="ai-empty-state">
                    ...
                    <p>
                      <strong>
                        ¡Hola! Soy el asistente de la Unidad Educativa
                      </strong>
                    </p>
                    <p>
                      {useLangChain
                    ...
                    <p style={{ fontSize: "0.85rem", marginTop: "12px" }}>
                      <i className="fas fa-lightbulb"></i> Usa los botones de
                      abajo para consultas rápidas
                    </p>
                  </div>
                ) : (
                  conversacion.map((msg, idx) => (
                    <div
                      key={idx}
                      className={`ai-message ${msg.autor === "user" ? "user" : "assistant"}`}
                    >
                      <div className="ai-message-bubble">
                        ...
                        <div
                          ...
                          dangerouslySetInnerHTML={{
                            __html: formatMessage(msg.texto),
                          }}
                        />
                        {msg.documentoAsistencia && (
                          <div
                            className="ai-documento-descarga"
                          ...
                          >
                            <button
                              ...
                              onClick={() =>
                                descargarDocumentoAsistencia(
                                  msg.documentoAsistencia,
                                )
                              }
                            >
                              <i
                                className={`fas ${msg.documentoAsistencia.formato === "word" || msg.documentoAsistencia.formato === "docx" ? "fa-file-word" : "fa-file-pdf"}`}
                              ></i>
                              Descargar{" "}
                              {msg.documentoAsistencia.formato === "word" ||
                              msg.documentoAsistencia.formato === "docx"
                                ? "Word"
                                : "PDF"}
                            </button>
                          </div>
      ...
      {historialWhatsappOpen && (
        <div
          ...
          onClick={(e) =>
            e.target === e.currentTarget && setHistorialWhatsappOpen(false)
          }
        >
          <div
