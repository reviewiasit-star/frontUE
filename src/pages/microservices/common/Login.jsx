...
import { checkAllServices } from '../../../config/apiConfig';
...
function Login({ onLogin }) {
  ...
  const [servicesStatus, setServicesStatus] = useState({ principal: null, cajas: null });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Primero intentar login (el servicio intentará ambos backends)
      const result = await AuthService.login(formData);

      if (result.ok) {
        const userRole = result.user.rol;
        
        // Después del login exitoso, verificar servicios para mostrar estado
        const services = await checkAllServices();
        setServicesStatus(services);
        
        // Validar disponibilidad de servicios según el rol
        if (userRole === 'Cajero' && !services.cajas) {
          setError('POR EL MOMENTO NO ESTÁ EN SERVICIO CAJAS ESTÁ EN MANTENIMIENTO');
          setSuccess(false);
          setLoading(false);
          // Limpiar autenticación si el servicio no está disponible
  ...
  // Verificar disponibilidad de servicios al cargar
  useEffect(() => {
    const checkServices = async () => {
      try {
        ...
        setServicesStatus({ principal: false, cajas: false });
      } finally {
              ...
              <div className="card-body p-4">
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  {!checkingServices && (
                    <div className="mb-3">
                      <div className="d-flex justify-content-between align-items-center small text-muted">
                        <span>
                          <i className={`fas fa-circle me-1 ${servicesStatus.principal ? 'text-success' : 'text-danger'}`}></i>
                          Administración
                        </span>
                        <span>
                          <i className={`fas fa-circle me-1 ${servicesStatus.cajas ? 'text-success' : 'text-danger'}`}></i>
                          Cajas
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="text-center mt-3">
                    <Link to="/recuperar-password" className="text-decoration-none" style={{ color: '#2a5298' }}>
