...
function Academia() {
  ...
  const cargarDatos = async () => {
    ...
    try {
      const [bloquesRes, nivelesRes, cursosRes, serviciosRes] = await Promise.all([
...
function GestionBloques({ bloques, niveles, cursos, showSuccess, showError, showWarning, onDataChange, onEditBloque, handleDropdownClick }) {
  ...
  const [deletingBloque, setDeletingBloque] = useState(false);

  const solicitarEliminarBloque = (bloque, e) => {
    e.stopPropagation();
  ...
  const cerrarDeleteModal = () => {
    if (deletingBloque) return;
    setShowDeleteModal(false);
    setBloqueToDelete(null);
  };

  const confirmarEliminarBloque = async () => {
    if (!bloqueToDelete?.id) return;
    setDeletingBloque(true);
    const token = localStorage.getItem('token');
    try {
      const response = await fetch(`${API_URL}/bloques/${bloqueToDelete.id}`, {
        method: "DELETE",
                      ...
                      <ul className="dropdown-menu" aria-labelledby={`dropdownBloque${bloque.id}`}>
                        ...
                        <li>
                          <button 
                            ...
                            onClick={(e) => solicitarEliminarBloque(bloque, e)}
                          >
            ...
            <div className="modal-content">
              <div className="modal-header">
                ...
                <button
                  type="button"
                  className="btn-close"
                  onClick={cerrarDeleteModal}
                  disabled={deletingBloque}
                ></button>
              </div>
              <div className="modal-body">
                <p className="mb-2">¿Seguro que deseas eliminar este bloque?</p>
                <p className="mb-2">
                  <strong>{bloqueToDelete?.descripcion || ''}</strong>
                </p>
                <p className="text-muted mb-0">Se eliminarán todos sus niveles y cursos.</p>
              </div>
              <div className="modal-footer">
                <button
                  type="button"
                ...
                <button
                  ...
                  onClick={confirmarEliminarBloque}
                  disabled={deletingBloque}
                >
                  {deletingBloque ? (
                    <>
...
function BloqueDetailModal({ isOpen, onClose, bloque, niveles, cursos, onDataChange, showSuccess, showError, showWarning, handleDropdownClick }) {
  ...
  const handleEditCurso = (curso) => {
    ...
    setCursoFormData({
      ...
      turno: curso.turno || '',
      hora_inicio: curso.hora_inicio ? String(curso.hora_inicio).slice(0, 5) : '',
      hora_fin: curso.hora_fin ? String(curso.hora_fin).slice(0, 5) : ''
    });
  ...
  const handleEditNivel = (nivel) => {
    ...
    setNivelFormData({
      ...
      precio: nivel.precio,
      meses: nivel.meses || ['febrero', 'marzo', 'abril', 'mayo', 'junio', 'julio', 'agosto', 'septiembre', 'octubre', 'noviembre']
    });
  ...
  useEffect(() => {
    if (editingCurso) {
      setCursoFormData({
        ...
        turno: editingCurso.turno || '',
        hora_inicio: editingCurso.hora_inicio ? String(editingCurso.hora_inicio).slice(0, 5) : '',
        hora_fin: editingCurso.hora_fin ? String(editingCurso.hora_fin).slice(0, 5) : ''
      });
      setShowCursoForm(true);
    } else {
      setCursoFormData({ nombre: '', nivel_id: '', turno: '', hora_inicio: '', hora_fin: '' });
                        ...
                        <form onSubmit={handleCursoSubmit}>
                          ...
                          <div className="row mt-3">
                            <div className="col-md-4">
                              ...
                              <select
                                className="form-select"
                                value={cursoFormData.turno}
                                onChange={(e) => setCursoFormData({ ...cursoFormData, turno: e.target.value })}
                              >
                            ...
                            <div className="col-md-4">
                              ...
                              <input
                                type="time"
                                className="form-control"
                                value={cursoFormData.hora_inicio}
                                onChange={(e) => setCursoFormData({ ...cursoFormData, hora_inicio: e.target.value })}
                              />
                            </div>
                            <div className="col-md-4">
                              ...
                              <input
                                type="time"
                                className="form-control"
                                value={cursoFormData.hora_fin}
                                onChange={(e) => setCursoFormData({ ...cursoFormData, hora_fin: e.target.value })}
                              />
                            </div>
                          </div>
                          <div className="mt-3">
                          ...
                          <div key={curso.id} className="col-md-6 col-lg-4 mb-3">
                            <div className="card border-info">
                              <div className="card-body">
                                ...
                                {(curso.turno || curso.hora_inicio || curso.hora_fin) && (
                                  <div className="mt-2">
                                    {curso.turno && <span className="badge bg-secondary me-2">{curso.turno}</span>}
                                    {(curso.hora_inicio || curso.hora_fin) && (
                                      <small className="text-muted">
                                        <i className="fas fa-clock me-1"></i>
                                        {curso.hora_inicio ? String(curso.hora_inicio).slice(0, 5) : '--:--'} - {curso.hora_fin ? String(curso.hora_fin).slice(0, 5) : '--:--'}
                                      </small>
                                    )}
                                  </div>
                                )}
                  ...
                  {!showCursoForm && (
                    <div className="mb-4">
                      <button 
                        ...
                        onClick={() => {
                          setCursoFormData({ nombre: '', nivel_id: selectedNivel.id, turno: '', hora_inicio: '', hora_fin: '' });
                          setShowCursoForm(true);
                        }}
                      >
...
function ServicioModal({ isOpen, onClose, servicio, onSuccess, showSuccess, showError }) {
  const [descripcion, setDescripcion] = useState('');
  const [loading, setLoading] = useState(false);
  const token = localStorage.getItem('token');

  useEffect(() => {
    if (servicio) setDescripcion(servicio.descripcion || '');
    else setDescripcion('');
  }, [servicio]);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const url = servicio ? `${API_URL}/servicios/${servicio.id}` : `${API_URL}/servicios`;
      ...
      if (!resp.ok || data.ok === false) {
        showError('Error', data.message || 'No se pudo guardar');
        return;
      }
      showSuccess('Éxito', `Servicio ${servicio ? 'actualizado' : 'creado'} correctamente`);
      onSuccess();
    } catch (err) {
            ...
            <div className="modal-body">
              <div className="mb-3">
                ...
                <input
                  ...
                  required
                  disabled={loading}
                  placeholder="Ej: Almuerzo, Tutoría, Refrigerio"
                />
              </div>
            </div>
            <div className="modal-footer">
