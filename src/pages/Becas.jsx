...
function Becas() {
  ...
  const eliminarBeca = async (id) => {
    if (window.confirm('¿Estás seguro de que quieres eliminar esta beca?')) {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`http://${window.location.hostname}:3001/api/becas/${id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
          }
        ...
        const data = await response.json();
        if (data.ok) {
          showSuccess('Beca eliminada', 'Beca eliminada exitosamente');
          cargarBecas();
        } else {
          showError('Error', data.message);
        }
                    ...
                    becas.map(beca => (
                      <tr key={beca.id}>
                        ...
                        <td>
                          ...
                          <button className="btn btn-sm btn-danger" onClick={() => eliminarBeca(beca.id)}>Eliminar</button>
                        </td>
                      </tr>
                    ))
                  )}
