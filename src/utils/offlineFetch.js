...
function getOriginalFetch() {
  ...
  throw new Error(
    "No se pudo obtener el fetch original. El sistema puede estar en un estado inconsistente.",
  );
}
...
export async function offlineFetch(url, options = {}) {
  ...
  if (!isGetRequest) {
    ...
    const esSubidaDocumento =
      urlStr.includes("documentos-agente/subir") &&
      options.body instanceof FormData;
    if (esSubidaDocumento) {
      throw new Error(
        "No se pudo subir el documento. Verifica que el servidor esté activo e inténtalo de nuevo.",
      );
    }

    // NO guardar requests de IA, chat o WhatsApp (no son idempotentes ni tienen sentido offline)
    const esRequestIA =
      urlStr.includes("/ai-admin") ||
      urlStr.includes("/ai-admin-langchain") ||
      urlStr.includes("/analisis-autonomo") ||
      urlStr.includes("/whatsapp") ||
      urlStr.includes("/historial-chat");
    if (esRequestIA) {
      throw new Error(
        "No se pudo conectar con el agente. Verifica tu conexión e inténtalo de nuevo.",
      );
    }
    ...
    try {
      ...
      try {
        ...
        if (endpoint.includes("/estudiantes")) {
          ...
          endpoint.includes("/pagos-realizados") ||
          endpoint.includes("/pagos") ||
          endpoint.includes("/pago")
        ) {
        ...
        console.log(
          `💾 ${tipoOperacion.toUpperCase()} guardado offline: ${method} ${endpoint}`,
        );
        console.log(
          `   ⏳ Pendiente de registro - Se sincronizará automáticamente cuando vuelva la conexión`,
        );
      ...
      if (typeof window !== "undefined") {
        window.dispatchEvent(
          new CustomEvent("offline-request-saved", {
      ...
      return new Response(
        JSON.stringify({
          ok: true,
          message:
            "Operación guardada. Se sincronizará automáticamente cuando vuelva la conexión.",
          offline: true,
          saved: true,
          requestId: requestId,
        }),
...
export async function syncPendingRequests() {
  const { getPendingRequests, deletePendingRequest } =
    await import("./offlineStorage");
  ...
  if (!backendAvailable) {
    console.log(
      `⏳ Backend no disponible - ${pending.length} operaciones pendientes se mantienen guardadas`,
    );
  ...
  for (const request of pending) {
    ...
    if (request.url && request.url.includes("documentos-agente/subir")) {
      ...
      console.log(
        `🗑️ Operación de documento eliminada (no sincronizable): ${request.url}`,
      );
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.warn(
          `⚠️ Falló sincronización (${response.status}): ${request.url} - Se reintentará`,
        );
        failed++;
      }
    } catch (error) {
  ...
  if (success > 0 || failed > 0) {
    console.log(
      `📤 Sincronización: ${success} exitosos, ${failed} pendientes de reintento`,
    );
  }
...
export function setupOfflineFetch() {
  ...
  window.fetch = async function (url, options = {}) {
    ...
    const isBuscarHijosRequest =
      urlString && urlString.includes("/buscar-hijos-remitente");

    // EXCLUIR endpoints de IA y WhatsApp - son lentos (Gemini puede tardar 30s+), no son idempotentes
    // y NO deben guardarse en IndexedDB si fallan (no tiene sentido reenviar un chat offline)
    const isAIRequest =
      urlString &&
      (urlString.includes("/ai-admin") ||
        urlString.includes("/ai-admin-langchain") ||
        urlString.includes("/analisis-autonomo") ||
        urlString.includes("/whatsapp") ||
        urlString.includes("/historial-chat"));

    // Para autenticación, búsqueda de hijos o IA, usar fetch original directamente (sin interceptación)
    if (isAuthRequest || isBuscarHijosRequest || isAIRequest) {
      return originalFetch(url, options);
    }
    ...
    if (navigator.onLine) {
      try {
        ...
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 15000); // 15 segundos timeout (Railway puede tardar al despertar)
        ...
        if (isGetRequest) {
          ...
          return new Response(
            JSON.stringify({
              ok: false,
              message: "Sin conexión y sin datos en cache",
            }),
            {
              status: 503,
    ...
    try {
      ...
      console.warn(
        "offlineFetch falló completamente, usando fetch original como último recurso:",
        error.message,
      );
      return originalFetch(url, options);
    }
  ...
  window.addEventListener("online", async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        if (pending.length > 0) {
          console.log(
            `📋 Intentando sincronizar ${pending.length} operaciones pendientes...`,
          );
          const result = await syncPendingRequests();
          if (result.pending > 0) {
            console.log(
              `⏳ ${result.pending} operaciones aún pendientes (backend no disponible o error)`,
            );
          }
        }
      } catch (error) {
        console.warn(
          "No se pudo sincronizar automáticamente - los datos se mantienen guardados",
        );
      }
    }, 5000);
  });
  ...
  setTimeout(async () => {
    try {
      ...
      if (pending.length > 0) {
        console.log(
          `📋 Hay ${pending.length} operaciones pendientes guardadas`,
        );
        console.log(
          `   💡 Usa syncOfflinePending() para sincronizar manualmente`,
        );
        console.log(
          `   💡 Usa clearOfflinePending() para limpiar (cuidado: elimina datos)`,
        );
      }
    } catch (error) {
      // Silenciar error
  ...
  window.syncOfflinePending = async () => {
    ...
    if (result.success === 0 && result.pending > 0) {
      console.log(
        `⚠️ No se pudo sincronizar - ${result.pending} operaciones siguen pendientes`,
      );
    }
    return result;
  };
